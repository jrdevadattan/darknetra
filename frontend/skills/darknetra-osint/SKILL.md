...
For a supplied website review in normal chat or Netra, read [site-review.md](references/site-review.md). Execute `site-review <url> [1–60]` for a linked-page request: it follows explicit public same-origin links and returns individual pages plus a coverage ledger. Default 30 reads, depth 4 and 180 seconds; share a total 60-page allowance across the team per user turn. Use `page-section <url> <nextOffset>` and `page-links <url> <nextLinkOffset>` to inspect relevant content beyond the first text/link window. Preserve actual retrieved/failed/skipped/pending outcomes, source quotes and explicit JavaScript/access/truncation warnings. No whole-site completeness is implied. Read [review-flows.md](references/review-flows.md) when source findings lead to wallet, access, monitoring or departmental-record questions.
...
For a relevant known-network Bitcoin address found in a case source, read [wallet-review.md](references/wallet-review.md) and execute `wallet-review bitcoin <address> [1–25]`. It returns bounded public transaction records and exact amounts with API provenance; it does not supply exchange attribution, account identity or model-ready features. Retain the source page and quotation where the wallet was found. Review at most three relevant addresses per turn; do not expand into every counterparty. Use [review-flows.md](references/review-flows.md) for missing records and independently sourced exchange leads. No account registration or wallet connection is part of this flow.
