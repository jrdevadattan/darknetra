...
export function WorkspaceShell({ initialCaseId }: { initialCaseId?: string }) {
  ...
  const [messages, setMessages] = useState<ThreadMessage[]>([]); const [snapshot, setSnapshot] = useState<ExecutionSnapshot | null>(null); const [notice, setNotice] = useState<Notice>(null); const [loading, setLoading] = useState(true); const [pending, setPending] = useState(false); const [createMode, setCreateMode] = useState<"case" | "thread" | "chat" | null>(null); const [draft, setDraft] = useState(""); const [harness, setHarness] = useState("DETERMINISTIC"); const [budget, setBudget] = useState("2");
  ...
  const create = async (event: FormEvent) => { event.preventDefault(); if (!draft.trim()) return; setPending(true); setNotice(null); try { if (createMode === "case") { const created = await api.createCase({ title: draft.trim(), demo: true, source_policy: { allowed_source_classes: ["SYNTHETIC"], tor_enabled: false, person_lookup_enabled: false, telegram_enabled: false } }); setCases((current) => [created, ...current]); setCaseId(created.id); router.replace(`/cases/${created.id}`); } else if (createMode === "thread" && caseId) { const created = await api.createThread(caseId, { title: draft.trim(), harness, budget_usd: Number(budget) }); setThreads((current) => [created, ...current]); setThreadId(created.id); } else if (createMode === "chat") { const created = await api.createChat({ title: draft.trim(), provider: harness === "NIM" ? "NIM" : harness === "OFFLINE" ? "OFFLINE" : "AUTO", budget_usd: Number(budget) }); setChats((current) => [created, ...current]); } setDraft(""); setCreateMode(null); } catch (error) { setNotice(noticeFor(error)); } finally { setPending(false); } };
  ...
  return <Flex h="100vh" minH="600px" direction="column" bg="bg">
    ...
    {loading ? <Flex flex="1" align="center" justify="center" gap="3"><Spinner color="teal.400" /><Text>Loading workspace…</Text></Flex> : <Flex flex="1" minH="0">
      <Box w={{ base: "244px", lg: "300px" }} flexShrink="0" borderRightWidth="1px" borderColor="border" bg="bg.panel" overflowY="auto"><Stack p="3" gap="4">
        ...
        <HStack gap="1"><Button size="xs" colorPalette="teal" onClick={() => setCreateMode("case")}><Plus size={14} /> New case</Button><Button size="xs" variant="outline" onClick={() => setCreateMode("chat")}><MessageSquare size={14} /> New chat</Button></HStack>
        {createMode && <Box borderWidth="1px" borderColor="teal.500" borderRadius="md" p="3"><Text fontWeight="semibold" fontSize="sm">{createMode === "case" ? "Create synthetic case" : createMode === "thread" ? "Create case thread" : "Create private chat"}</Text><form onSubmit={create}><Stack gap="2" mt="2"><Input size="sm" value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={createMode === "case" ? "Case title" : "Conversation title"} autoFocus /><HStack><Text fontSize="xs" color="fg.muted">Runtime</Text><select aria-label="Runtime" value={harness} onChange={(event) => setHarness(event.target.value)}><option value="DETERMINISTIC">Deterministic (offline)</option><option value="OFFLINE">Ollama / offline</option><option value="NIM">NVIDIA NIM</option><option value="CLAUDE">Claude</option></select><Input aria-label="Budget USD" size="sm" type="number" min="0.01" max="1000" step="0.01" value={budget} onChange={(event) => setBudget(event.target.value)} w="80px" /></HStack><HStack justify="end"><Button size="xs" variant="ghost" onClick={() => setCreateMode(null)}>Cancel</Button><Button size="xs" type="submit" colorPalette="teal" loading={pending}>Create</Button></HStack></Stack></form></Box>}
        ...
        <Separator /><HStack justify="space-between"><Text fontSize="xs" textTransform="uppercase" color="fg.muted" fontWeight="bold">Case threads</Text>{caseId && <IconButton aria-label="New case thread" size="xs" variant="ghost" onClick={() => setCreateMode("thread")}><Plus size={14} /></IconButton>}</HStack><Stack gap="1">{threads.length === 0 && <Text fontSize="sm" color="fg.muted">No threads for this case.</Text>}{threads.map((item) => <Button key={item.id} justifyContent="start" textAlign="left" variant={item.id === threadId ? "subtle" : "ghost"} colorPalette={item.id === threadId ? "teal" : "gray"} onClick={() => setThreadId(item.id)}><MessageSquare size={16} /><Text truncate>{item.title}</Text></Button>)}</Stack>
