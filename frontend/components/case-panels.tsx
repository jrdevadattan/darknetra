...
export function CasePanels({ caseId }: { caseId: string | null }) {
  ...
  useEffect(() => { if (!caseId) return; setLoading(true); setError(null); setData(null); const selected = surfaces.find((item) => item.id === surface); const call = surface === "overview" ? api.summary(caseId) : surface === "monitoring" ? api.monitoring(caseId) : surface === "plugins" ? Promise.all([api.plugins(caseId), api.tools()]) : api.panel(caseId, selected?.path ?? surface); void call.then(setData).catch((reason) => setError(errorText(reason))).finally(() => setLoading(false)); }, [caseId, surface]);
  const records = Array.isArray(data) ? data.flatMap((item) => (item as { items?: Array<Record<string, unknown>> })?.items ?? []) : ((data as { items?: Array<Record<string, unknown>> } | null)?.items ?? []);
  const body = !caseId ? <Text color="fg.muted" fontSize="sm">Choose a case to inspect its records.</Text> : loading ? <HStack color="fg.muted"><Spinner size="sm" /><Text>Loading {title.toLowerCase()}…</Text></HStack> : error ? <Box borderWidth="1px" borderColor="orange.400" borderRadius="md" p="3"><Text fontWeight="semibold">Panel unavailable</Text><Text fontSize="sm" color="fg.muted">{error}</Text></Box> : surface === "overview" ? <Summary data={data as Record<string, unknown> | null} /> : surface === "graph" ? <Graph data={data as Record<string, unknown> | null} /> : <Records rows={records} empty={`No ${title.toLowerCase()} records are available for this case.`} />;
