...
**Settings → General → Language** offers English, Hindi and Punjabi first, followed by the remaining 20 scheduled Indian languages. The choice is saved in the local workspace and applies to core interface labels and future assistant replies, including scheduled reports. Each reply records its language; changing the setting does not rewrite history or switch an in-progress reply. Original evidence quotations and identifiers stay intact. Urdu, Sindhi and Kashmiri use right-to-left text. Manipuri uses Bengali script and Santali uses Ol Chiki. Bundled UI translations make no external translation requests; longer help text, some specialist controls and technical errors currently retain English fallbacks.
