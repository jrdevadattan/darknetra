...
def test_ingest_task_maps_attempts_to_authoritative_job_status() -> None:
    assert IngestTask.persistence_on_retry(0) == (1, JobStatus.RETRYING)
    assert IngestTask.persistence_on_retry(3) == (4, JobStatus.RETRYING)
    assert IngestTask.persistence_on_terminal_failure(4) == (5, JobStatus.FAILED)
...
def test_non_retryable_ingest_failure_is_terminal_on_first_attempt() -> None:
    application = Celery("ingest-terminal-policy-test", task_cls=IngestTask)
    application.conf.update(task_always_eager=True, task_eager_propagates=False)
    attempts: list[int] = []

    @application.task
    def fails_terminally() -> None:
        attempts.append(len(attempts) + 1)
        raise NonRetryableIngestError("synthetic terminal failure")

    result = fails_terminally.apply()

    assert result.failed()
    assert attempts == [1]
    assert IngestTask.persistence_on_terminal_failure(0) == (1, JobStatus.FAILED)
